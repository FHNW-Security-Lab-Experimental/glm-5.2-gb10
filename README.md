...
| **`dcp-512k`** | 512k | ~22–23 | **~9 × 512k** (8 slots) | MLA KV sharded 8-way |
| **`dcp-1m`** | **1,048,576** | ~18–19 | **4 × 1M** (vLLM: 4.12×) | MLA KV sharded 8-way |

¹ single-stream, warm, GB10, real-token (`completion_tokens`; MTP packs ~2.8 tok/SSE-chunk).
The DCP decode is **PERF#2** (each rank's local top-2048 → fixed-size all-gather → exact global
top-2048): **+12%** over the original all-gather-full-logits reassembly (validated real-token
16.9→19.0 tok/s @ 256k, correctness identical), now the production DCP path. The original decode
is the one-file rollback `8node-nvfp4/glm52-dcp-patches-itemb.sh`. Slots ≈
`min(--max-num-seqs, KV-pool ÷ context)`. Full table + the perf writeup:
**[`8node-nvfp4/CONFIGS.md`](8node-nvfp4/CONFIGS.md)**, **[`8node-nvfp4/PERF-RESULTS.md`](8node-nvfp4/PERF-RESULTS.md)**.
...
- **+12% decode (PERF#2, production)** — the DCP decode picks the global top-2048 by each rank taking
  its LOCAL top-2048 then a **fixed-size** all-gather + union (not all-gathering the full per-key
  logits), removing the always-1M-wide per-step indexer tax. Validated real-token (16.9→19.0 tok/s @
  256k, correctness identical: needle 32k/131k + deterministic A/B). Offline-proven exact + a 4-lens
  adversarial review. See `8node-nvfp4/PERF2-DECODE-SPEC.md` + `8node-nvfp4/PERF-RESULTS.md` (which
  also records the PERF#1 prefill-cache **NO-GO** — a multi-chunk-prefill deadlock — and the
  launcher hardening). Rollback: `8node-nvfp4/glm52-dcp-patches-itemb.sh`.
...
The whole 8× runtime is in `8node-nvfp4/`: the (hardened) launcher (`start_glm52_8node.sh`), the
3-config starter (`start_glm52_config.sh`), the base patches (`glm52-sparse-patches.sh`) + the DCP
A/B/C patches (`glm52-dcp-patches.sh` = the **PERF#2 production decode**; original decode preserved
as `glm52-dcp-patches-itemb.sh`) + the combined-patch builder (`build-dcp-patch.sh`, which
regenerates the in-container patch from those sources), all Triton kernels (`kernels/`), the
watchdog (`watchdog_glm52_cluster.sh` + `systemd/`), the validators + benchmarks
(`validate_dcp.py`, `validate_dcp_reassembly.py`, `validate_lse_recombine.py`,
`validate_perf2_decode.py`, `validate_ab_equivalence.py`, `measure_decode.py`, `verify_anchors.py`),
the perf writeups (`PERF-PLAN.md`, `PERF-RESULTS.md`, `PERF2-DECODE-SPEC.md`), the b12x wheel
(`wheels/`), and the model-distribution helper (`scripts/`). The combined patch
(`glm52-sparse-patches-dcp.sh`) is regenerated by `build-dcp-patch.sh`, not committed.
