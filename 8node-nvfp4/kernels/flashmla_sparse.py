...
class FlashMLASparseImpl(SparseMLAAttentionImpl[FlashMLASparseMetadata]):
    # GLM52_DCP: the sparse decode path can return a per-shard base-e softmax LSE,
    # so vLLM's MLA context-parallel path (mla_attention.py ->
    # cp_lse_ag_out_rs / dcp_a2a_lse_reduce) can recombine attention across
    # decode-context-parallel KV shards. Only used when dcp_world_size > 1; the
    # non-DCP production path is byte-for-byte unchanged. See 1M-SPARSE-CP-KV-PLAN.md.
    ...
    def _forward_fp8_kv_separate_prefill_decode(
        ...
        # GLM52_DCP: return_lse=True also returns the per-shard base-e LSE
        # [num_actual_tokens, num_heads] for context-parallel recombine. The non-DCP
        # production path (return_lse=False) is byte-for-byte unchanged.
        ...
        def _fp8_decode(
            ...
            out = reshape_attn_output_for_spec_decode(attn_out)
            if want_lse:
                # GLM52_DCP: lse [num_decodes, H, seq_len] -> [num_decode_tokens, H]
                # (the [B, H] base-e layout cp_lse_ag_out_rs expects).
                return out, lse.transpose(1, 2).reshape(
        ...
        if num_decode_tokens > 0 and num_prefill_tokens == 0:
            assert fp8_metadata.decode is not None
            if return_lse:
                attn_out, dec_lse = _fp8_decode(q, topk_indices, want_lse=True)
                return attn_out, dec_lse
            ...
            if num_decode_tokens > 0:
                if return_lse:
                    dec_out, dec_lse = _fp8_decode(
                        q[:num_decode_tokens],
                        topk_indices[:num_decode_tokens],
                        want_lse=True,
                    )
                    attn_out[:num_decode_tokens] = dec_out
                    lse_full[:num_decode_tokens] = dec_lse
                else:
            ...
            for chunk in fp8_metadata.prefill.chunks:
                ...
                if return_lse:
                    ...
                    lse_full[chunk.tokens_slice] = chunk_lse
                else:
    ...
    def _forward_fp8_kv_mixed_batch(
        ...
        if return_lse:
            # GLM52_DCP: lse (1, H, T) -> (T, H) base-e for cp_lse_ag_out_rs.
            return attn_out, lse.squeeze(0).transpose(0, 1).contiguous()
        return attn_out
    ...
    def _bf16_flash_mla_kernel(
        ...
        # flash_mla_sparse_fwd (patched to flash_mla_sparse_fwd_triton) returns
        # (out [s_q, h_q, d_v], max_logits [s_q, h_q], lse [s_q, h_q]); lse is base-e.
        ...
        output = result[0][:, : self.num_heads, :]
        if return_lse:
            # GLM52_DCP: per-shard base-e LSE [s_q, num_heads] for cp recombine.
            return output, result[2][:, : self.num_heads].contiguous()
        return output
    ...
    def forward_mqa(
        ...
        # GLM52_DCP: under context parallelism (decode-context-parallel-size > 1)
        # EVERY mqa step (prefill AND decode) must return the per-shard base-e LSE so
        # mla_attention.py can recombine attention across KV shards via
        # cp_lse_ag_out_rs / dcp_a2a_lse_reduce. Both fp8 paths thread the LSE now
        # (the sparse prefill kernel flash_mla_sparse_fwd already computes it). Non-DCP
        # serving never enters this branch, so the production path is unchanged.
        want_lse = getattr(self, "dcp_world_size", 1) > 1
        if want_lse:
            if not use_fp8_cache:
                raise NotImplementedError(
                    "GLM52_DCP: bf16 KV under DCP not supported (use fp8_ds_mla)."
                )
            if attn_metadata.fp8_use_mixed_batch:
                return self._forward_fp8_kv_mixed_batch(
                    q, kv_c_and_k_pe_cache, topk_indices, attn_metadata, return_lse=True
                )
            return self._forward_fp8_kv_separate_prefill_decode(
                q, kv_c_and_k_pe_cache, topk_indices, attn_metadata, return_lse=True
            )
